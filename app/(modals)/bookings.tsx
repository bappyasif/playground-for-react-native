      ...
      <View style={styles.card}>
        {
          openCard !== 0
            ? (
              <AnimatedTouchableOpacity onPress={() => setOpenCard(0)} style={styles.previewCard} entering={FadeIn.duration(400)} exiting={FadeOut.duration(200)}>
            ...
            : (
              <>
                <Animated.Text style={styles.cardHeader} entering={FadeIn} exiting={FadeOut}>Where to?</Animated.Text>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 22, paddingLeft: 20, marginBottom: 31 }}>
                  {places.map((item, idx) => (
                    <TouchableOpacity key={idx} onPress={() => setSelectedPlace(idx)}>
                      {/* <Image source={{uri: item.img}} style={selectedPlace === idx ? styles.placeSelected : styles.place} /> */}
      ...
      <View style={styles.card}>
        {
          openCard !== 1
            ? (
              <AnimatedTouchableOpacity onPress={() => setOpenCard(1)} style={styles.previewCard} entering={FadeIn.duration(400)} exiting={FadeOut.duration(200)}>
            ...
            : (
              <>
                <Animated.Text style={styles.cardHeader} entering={FadeIn} exiting={FadeOut}>When is your trip?</Animated.Text>
                <Animated.View style={styles.cardBody}>
                  <DatePicker
                    current={today}
                    selected={today}
      ...
      <View style={styles.card}>
        {
          openCard !== 2
            ? (
              <AnimatedTouchableOpacity onPress={() => setOpenCard(2)} style={styles.previewCard} entering={FadeIn.duration(400)} exiting={FadeOut.duration(200)}>
            ...
            : (
              <>
                <Animated.Text style={styles.cardHeader} entering={FadeIn} exiting={FadeOut}>Who's coming ?</Animated.Text>
                <Animated.View style={styles.cardBody}>
                  {groups?.map((item, idx) => (
                    <View key={idx} style={[styles.guestItem, idx + 1 < guestsGroups.length ? styles.itemBorder : null]}>
                      ...
                      <View style={{ flexDirection: "row", gap: 10, alignItems: "center", justifyContent: "center" }}>
                        ...
                        <Text style={{ fontFamily: "mont-r", fontSize: 15, minWidth: 18, textAlign: "center" }}>{item.count}</Text>
      ...
      <Animated.View style={defaultStyles.footer} entering={SlideInDown.delay(200)}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <TouchableOpacity onPress={onClearAll} style={{ justifyContent: "center" }}>
            <Text style={{ fontSize: 18, fontFamily: "mont-sb", textDecorationLine: "underline" }}> Clear all</Text>
          </TouchableOpacity>
